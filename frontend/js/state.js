...
export async function garantisciSquadre() { if (!cache.squadre.length) await caricaSquadre(); return cache.squadre; }
export async function garantisciCorridori() { if (!cache.corridori.length) await caricaCorridori(); return cache.corridori; }
export async function garantisciTappe() { if (!cache.tappe.length) await caricaTappe(); return cache.tappe; }
export async function garantisciNazioni() { if (!cache.nazioni.length) await caricaNazioni(); return cache.nazioni; }
export async function garantisciSponsor() { if (!cache.sponsor.length) await caricaSponsor(); return cache.sponsor; }
