...
function opzioniPer(tipo) {
  ...
  if (tipo === 'corridore') return cache.corridori.map(c => ({ value: c.id, label: `${c.nome} ${c.cognome}` }));
  if (tipo === 'tappa') return cache.tappe.map(t => ({ value: t.id, label: `Tappa ${t.numero_tappa} — ${t.nome}` }));
  if (tipo === 'sponsor') return cache.sponsor.map(s => ({ value: s.id, label: s.nome }));
  return [];
}

function risolviValore(colonna, valore) {
  if (valore === null || valore === undefined || valore === '') return '—';
  if (colonna.type === 'squadra') return cache.squadre.find(s => s.id === valore)?.nome ?? valore;
...
export function montaListaConForm(contenitore, cfg) {
  ...
  function apriForm(rigaEsistente) {
    ...
    const html = campiVisibili.map(c => campoHtml(c, 'cd_' + c.key, rigaEsistente ? rigaEsistente[c.key] : '')).join('');
    ...
    document.getElementById('cd_salva').addEventListener('click', async () => {
      ...
      if (res.ok) { chiudiModal(); mostraToast(rigaEsistente ? 'Modifiche salvate' : 'Salvato'); ricarica(); }
