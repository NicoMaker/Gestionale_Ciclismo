...
async function renderArrivo(corpo) {
  ...
  if (!tappaSelezionataId && cache.tappe.length) tappaSelezionataId = cache.tappe[0].id;
  ...
  sel.addEventListener('change', () => { tappaSelezionataId = +sel.value; ricaricaArrivo(); });
...
async function ricaricaArrivo() {
  ...
  tbody.querySelectorAll('[data-elimina]').forEach(b => b.addEventListener('click', () => eliminaRisultato(+b.dataset.elimina)));
}
...
function apriFormRisultato(risultatoEsistente) {
  ...
  document.getElementById('r_salva').addEventListener('click', () => salvaRisultato(risultatoEsistente));
}
...
async function salvaRisultato(risultatoEsistente) {
  ...
  if (res.ok) { chiudiModal(); mostraToast(risultatoEsistente ? 'Risultato modificato' : 'Risultato salvato'); }
...
export function init(container) {
  ...
  socket.on('risultati:aggiornati', () => { if (sottoTabAttiva === 'arrivo') ricaricaArrivo(); });
}
