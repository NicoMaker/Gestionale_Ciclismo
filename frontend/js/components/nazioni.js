...
import { apriModal, chiudiModal, mostraToast, bandiera, attivaCampoRicerca } from '../utils.js';
...
function disegna(tbody) {
  const filtrate = nazioniCorrenti.filter(n =>
    !queryCorrente || n.nome.toLowerCase().includes(queryCorrente) || n.codice_iso2.toLowerCase().includes(queryCorrente)
  );
  tbody.innerHTML = filtrate.map(n => `
    <tr>
  ...
  `).join('') || `<tr><td colspan="3" style="text-align:center;color:#999;padding:24px;">${queryCorrente ? 'Nessuna nazione trovata' : 'Nessuna nazione inserita'}</td></tr>`;
...
export function init(container) {
  ...
  attivaCampoRicerca(corpo, q => { queryCorrente = q; disegna(tbody); });
