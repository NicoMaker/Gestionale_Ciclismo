...
import { creaSottoSchede, htmlCampoRicerca, attivaCampoRicerca } from '../utils.js';
...
async function renderGenerale(corpo) {
  ...
  `;
  attivaCampoRicerca(corpo, q => { queryCorrente = q; disegnaGenerale(); });
...
function disegnaGenerale() {
  ...
  tbody.innerHTML = filtrati.map((r, i) => {
  ...
  `;
  }).join('') || `<tr><td colspan="5" style="text-align:center;color:#999;padding:24px;">${queryCorrente ? 'Nessun corridore trovato' : 'Nessun dato disponibile'}</td></tr>`;
}
