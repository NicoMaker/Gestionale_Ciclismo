...
module.exports = (io) => {
  ...
  router.get('/:id', (req, res) => {
    db.get('SELECT * FROM squadre WHERE id = ?', [req.params.id], (err, row) => {
      if (err) return res.status(500).json({ errore: err.message });
      if (!row) return res.status(404).json({ errore: 'Squadra non trovata' });
      res.json(row);
    });
  });
  ...
  router.put('/:id', (req, res) => {
    ...
    db.run(
      ...
      function (err) {
        if (err) return res.status(400).json({ errore: err.message });
        if (this.changes === 0) return res.status(404).json({ errore: 'Squadra non trovata' });
        io.emit('squadre:aggiornate', { tipo: 'modificata', id: req.params.id });
