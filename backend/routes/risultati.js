...
module.exports = (io) => {
  ...
  router.post('/', (req, res) => {
    const { tappa_id, corridore_id, posizione, tempo, distacco, punti } = req.body;
    if (!tappa_id || !corridore_id) {
      return res.status(400).json({ errore: 'tappa_id e corridore_id sono obbligatori' });
    }
    const sql = `
    ...
    `;
    db.run(sql, [tappa_id, corridore_id, posizione || null, tempo || null, distacco || '00:00:00', punti || 0],
      function (err) {
        if (err) return res.status(400).json({ errore: err.message });
        const risultato = { tappa_id, corridore_id, posizione, tempo, distacco, punti };
  ...
  router.delete('/:id', (req, res) => {
    db.run('DELETE FROM risultati WHERE id = ?', [req.params.id], function (err) {
      if (err) return res.status(400).json({ errore: err.message });
      if (this.changes === 0) return res.status(404).json({ errore: 'Risultato non trovato' });
      io.emit('risultati:aggiornati', { tipo: 'eliminato', id: req.params.id });
      res.json({ ok: true });
    });
  });
