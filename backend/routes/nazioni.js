...
module.exports = (io) => {
  ...
  router.get('/ricerca', (req, res) => {
    ...
    db.all('SELECT * FROM nazioni WHERE lower(nome) LIKE ? ORDER BY nome LIMIT 15', [q], (err, rows) => {
      if (err) return res.status(500).json({ errore: err.message });
      res.json(rows);
    });
  });
  ...
  router.post('/', (req, res) => {
    ...
    db.run('INSERT INTO nazioni (nome, codice_iso2) VALUES (?, ?)', [nome, codice_iso2.toUpperCase()], function (err) {
      if (err) return res.status(400).json({ errore: err.message });
      const nuova = { id: this.lastID, nome, codice_iso2: codice_iso2.toUpperCase() };
