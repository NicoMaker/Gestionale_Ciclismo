...
module.exports = (io) => {
  ...
  router.post('/', (req, res) => {
    const { numero_tappa, nome, partenza, arrivo, distanza_km, dislivello_m, tipo, data, stato } = req.body;
    if (!numero_tappa || !nome || !partenza || !arrivo) {
      return res.status(400).json({ errore: 'numero_tappa, nome, partenza e arrivo sono obbligatori' });
    }
    db.run(
      `INSERT INTO tappe (numero_tappa, nome, partenza, arrivo, distanza_km, dislivello_m, tipo, data, stato)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [numero_tappa, nome, partenza, arrivo, distanza_km || null, dislivello_m || null,
       tipo || 'pianura', data || null, stato || 'programmata'],
      function (err) {
        if (err) return res.status(400).json({ errore: err.message });
        const nuova = { id: this.lastID, numero_tappa, nome, partenza, arrivo, distanza_km, dislivello_m, tipo, data, stato };
  ...
  router.put('/:id', (req, res) => {
    ...
    db.run(
      `UPDATE tappe SET numero_tappa=?, nome=?, partenza=?, arrivo=?, distanza_km=?, dislivello_m=?, tipo=?, data=?, stato=?
       WHERE id=?`,
      [numero_tappa, nome, partenza, arrivo, distanza_km, dislivello_m, tipo, data, stato, req.params.id],
      function (err) {
        if (err) return res.status(400).json({ errore: err.message });
        if (this.changes === 0) return res.status(404).json({ errore: 'Tappa non trovata' });
        io.emit('tappe:aggiornate', { tipo: 'modificata', id: req.params.id });
        res.json({ id: req.params.id, numero_tappa, nome, partenza, arrivo, distanza_km, dislivello_m, tipo, data, stato });
      }
    );
  });
